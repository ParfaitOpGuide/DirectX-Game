...
Mesh::Mesh(const wchar_t* full_path) : Resource(full_path)
{
	...
	if (!err.empty())
		throw std::exception("Mesh not loaded");
...
Mesh::Mesh(const wchar_t* full_path, bool hastc, bool hasn) : Resource(full_path)
{
	tinyobj::attrib_t attribs;
	std::vector<tinyobj::shape_t> shapes;
	std::vector<tinyobj::material_t> materials;
	...
	for (size_t s = 0; s < shapes.size(); s++)
	{
		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
		{
			...
			for (unsigned char v = 0; v < num_face_verts; v++)
			{
				...
				if (hasn) {
					nx = attribs.vertices[index.normal_index * 3 + 0];
					ny = attribs.vertices[index.normal_index * 3 + 1];
					nz = attribs.vertices[index.normal_index * 3 + 2];
				}
