...
ResourcePtr ResourceManager::createResourceFromFile(const wchar_t* file_path, bool hastc, bool hasn)
{
	std::wstring full_path = std::filesystem::absolute(file_path);
	...
	Resource* raw_res = this->createResourceFromFileConcrete(full_path.c_str(), hastc, hasn);
