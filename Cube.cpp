...
void Cube::update(float deltaTime, float width, float height, Camera cam, bool isPers)
{
	...
	if (matset && getComponentsOfTypeExists((AComponent::ComponentType)3)) {
		cc.m_world = currMat;
	}
...
void Cube::draw(float width, float height, VertexShaderPtr m_vs, PixelShaderPtr m_ps, float deltaTime, std::vector<Camera> camList, int currentCam)
{
	ticked = false;
