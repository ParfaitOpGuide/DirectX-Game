...
public:
	MeshObject();
	MeshObject(float width, float height, float depth, float centerx, float centery, float centerz, Vector3D speed, Vector3D rotation, std::vector<Vector3D> colors, std::vector<Vector3D> colors2, string name, RasterState* m_raster, MeshPtr mesh);
	...
	void createBuffer(MeshObject copy);
	void destroy();
